...
function Logo() { return <Link className="inline-flex items-center rounded-control" to="/" aria-label="Laybrotech home"><img className="h-auto w-[124px] object-contain sm:w-[146px] lg:w-[156px]" src={logoUrl} alt="Laybrotech" width="156" height="48" /></Link>; }
...
export function Header() {
  ...
  return (
    <header className={cn('sticky top-0 z-50 border-b bg-white transition-[box-shadow,border-color] duration-smooth', isScrolled ? 'border-brand-border shadow-[0_8px_26px_rgb(23_23_23/0.06)]' : 'border-transparent')}>
      <PageContainer><div className="flex h-[72px] items-center justify-between gap-4 lg:h-20"><Logo />
        <nav className="hidden items-center gap-1 lg:flex" aria-label="Main navigation">
          <NavLink className={navLinkClass} to="/hosting">Web Hosting</NavLink>
          <Dropdown refEl={dropdownRef} buttonId={servicesButtonId} menuId={servicesMenuId} label="Services" items={serviceItems} isOpen={isServicesOpen} setOpen={setIsServicesOpen} isActive={isServiceRoute} firstRef={firstServiceRef} />
          <NavLink className={navLinkClass} to="/projects">Projects</NavLink>
          <NavLink className={navLinkClass} to="/blog">Explore More</NavLink>
        </nav>
      ...
      <div id={mobileMenuId} className={cn('fixed inset-x-0 top-[72px] z-40 h-[calc(100dvh-72px)] overflow-y-auto border-y border-[#e5e1dc] bg-[#fff] px-5 py-5 shadow-[0_20px_45px_rgb(23_23_23/0.16)] transition-all duration-smooth lg:hidden', isMobileOpen ? 'pointer-events-auto translate-y-0 opacity-100' : 'pointer-events-none -translate-y-2 opacity-0')}>
        <nav className="mx-auto flex h-full max-w-container flex-col rounded-card bg-[#fff]" aria-label="Mobile navigation">
          <NavLink className={({ isActive }) => cn('mb-2 rounded-button bg-[#fff] px-4 py-3 font-semibold transition-colors duration-smooth hover:bg-[#f2f2ef]', isActive ? 'text-brand-orange' : 'text-[#18181b]')} to="/hosting" onClick={closeMobileMenu}>Web Hosting</NavLink>
          <MobileGroup label="Services" items={serviceItems} open={isMobileServicesOpen} setOpen={setIsMobileServicesOpen} active={isServiceRoute} close={closeMobileMenu} />
          <NavLink className={({ isActive }) => cn('mt-2 rounded-button bg-[#fff] px-4 py-3 font-semibold transition-colors duration-smooth hover:bg-[#f2f2ef]', isActive ? 'text-brand-orange' : 'text-[#18181b]')} to="/projects" onClick={closeMobileMenu}>Projects</NavLink>
          <NavLink className={({ isActive }) => cn('mt-2 rounded-button bg-[#fff] px-4 py-3 font-semibold transition-colors duration-smooth hover:bg-[#f2f2ef]', isActive ? 'text-brand-orange' : 'text-[#18181b]')} to="/blog" onClick={closeMobileMenu}>Explore More</NavLink>
