...
export function BlogCard({ post }: { post: PublicBlogPost }) {
  ...
  return (
    <article className="group flex min-h-full flex-col overflow-hidden rounded-[1.35rem] border border-[#e7e2dd] bg-white shadow-[0_14px_36px_rgb(24_24_27/0.05)] transition-colors duration-200 hover:border-[#f25a05]/35">
      <Link className="overflow-hidden bg-[#f7f7f6]" to={`/blog/${post.slug}`}><img className="aspect-[4/3] w-full object-cover object-center transition duration-500 group-hover:scale-[1.018]" src={post.featured_image_url ?? ''} alt={post.featured_image_alt ?? ''} loading="lazy" decoding="async" /></Link>
      <div className="flex flex-1 flex-col p-6 sm:p-7">
        ...
        <p className="mt-4 text-xs font-bold uppercase text-[#766e67]">{formatDate(post.published_at ?? post.scheduled_at)} - {calculateReadTime(post.content)} min read</p>
